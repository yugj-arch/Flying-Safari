...
export default function FlyingSafari() {
  ...
  const styles = {
    ...
    // BOOKING WIDGET
    bookingWidget: { position: "absolute", bottom: 0, left: "5%", right: "5%", zIndex: 3, maxWidth: 1200, margin: "0 auto" },
        ...
        <div style={styles.footerTop} className="footer-top">
          <div>
            ...
            <div style={styles.footerSocial}>
              {["IG", "FB", "IN", "𝕏"].map((s, i) => (
                <div key={i} style={styles.socialBtn} onClick={() => showToast(`Opening ${s} profile`)}><span style={{ fontSize: 11 }}>{s}</span></div>
              ))}
            </div>
          </div>
          <div>
